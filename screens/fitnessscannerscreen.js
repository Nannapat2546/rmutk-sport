...
export default function FitnessScannerScreen({ navigation }) {
  ...
  const processScannedCode = async (code) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/users/scan/${code}`);
      ...
      if (response.ok) {
        ...
        let idCardImg = null;
        try {
          const detailRes = await fetch(`http://localhost:3000/api/admin/users/${result.id}/detail`);
          if (detailRes.ok) {
  ...
  const handleCaptureCard = async () => {
    ...
    try {
      ...
      setLoadingText('กำลังตรวจสอบข้อมูลในระบบ...');
      const response = await fetch(`http://localhost:3000/api/users/scan/${citizenId}`);
      const result = await response.json();
