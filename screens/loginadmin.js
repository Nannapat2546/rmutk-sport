...
export default function LoginScreen({ navigation }) {
  ...
  // 🌟 1. เพิ่ม State สำหรับเก็บข้อมูลแอดมินชั่วคราวตอนล็อกอินผ่าน
  ...
  const closePopup = () => {
    ...
    // 🌟 2. ถ้าเข้าสู่ระบบสำเร็จ ให้เด้งไปหน้า Dashboard "พร้อมกับส่งข้อมูลแอดมินไปด้วย"
    if (popupType === 'success') {
      navigation.replace('AdminHome', { 
        userData: adminData, 
        accountId: adminData?.id || adminData?.account_id 
      });
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // 🌟 3. เก็บข้อมูลที่ได้จากหลังบ้านไว้ใน State
      ...
      >
        ...
        <View style={styles.formContainer}>
          <View style={styles.card}>
            ...
            <Text style={styles.title}>ผู้ดูแลระบบ (Admin)</Text>
