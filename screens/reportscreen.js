...
export default function ReportScreen({ navigation }) {
  ...
  // 🌟 เพิ่ม State สำหรับเก็บ Index ของกราฟที่ถูกกด (Tooltip)
  const [selectedBarIndex, setSelectedBarIndex] = useState(null);

  useEffect(() => {
    // รีเซ็ตการกดกราฟทุกครั้งที่เปลี่ยนโหมดหรือเดือน
          ...
          <>
            ...
            <View style={styles.chartsRow}>
              ...
              <View style={styles.barChartCard}>
                ...
                {/* 🌟 พื้นที่กราฟแท่ง (เพิ่ม Padding บนเพื่อให้มีที่โชว์ Tooltip) */}
                    ...
                    {stats?.chartBar && stats.chartBar.map((item, index) => {
                      ...
                      return (
                        <TouchableOpacity 
                          key={index} 
                          activeOpacity={0.8}
                          // 🌟 เมื่อกดที่แท่งกราฟ ให้บันทึก index เพื่อแสดง Tooltip
...
const styles = StyleSheet.create({
  ...
  // 🌟 เพิ่ม paddingTop เพื่อกันไม่ให้กราฟที่เต็ม 100% บัง Tooltip ขาด
